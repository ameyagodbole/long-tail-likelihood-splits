...
from semantic_parsing.spider.process_sql import tokenize, get_schema, get_tables_with_alias, Schema, get_sql
...
def eval_internal(glist, plist, db_dir, etype, kmaps, silent=True):
    evaluator = Evaluator()
    ...
    for p, g in zip(plist, glist):
        ...
        try:
            ...
            # https://github.com/taoyds/spider/issues/52
            print("Skipping gold query: {}".format(g_sql))
            continue
        ...
        if etype in ["all", "match"]:
            ...
            if exact_score == 0 and not silent:
...
def evaluate(gold, predict, db_dir, etype, kmaps):
    ...
    with open(predict) as f:
        plist = [l.strip().split('\t')[0] for l in f.readlines() if len(l.strip()) > 0]
    # plist = [("select max(Share),min(Share) from performance where Type != 'terminal'", "orchestra")]
    # glist = [("SELECT max(SHARE) ,  min(SHARE) FROM performance WHERE TYPE != 'Live final'", "orchestra")]
    scores, entries = eval_internal(glist, plist, db_dir, etype, kmaps, silent=False)

    per_q_eval_filenm = os.path.splitext(predict)[0] + '-per_q_eval_result.json'
    with open(per_q_eval_filenm, 'w') as fout:
